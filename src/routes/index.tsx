...
const usableObjects = [
  { id: 'compass', name: 'Brújula de Yara', description: 'Revela el sendero más cercano durante unos instantes.', action: 'Revelar ruta', icon: Compass, accent: 'text-primary' },
  { id: 'leaf', name: 'Hoja de Yara', description: 'Una hoja viva que cambia de color cerca de reliquias.', action: 'Buscar reliquias', icon: Leaf, accent: 'text-emerald-300' },
  { id: 'lantern', name: 'Linterna de luciérnagas', description: 'Ilumina pistas ocultas cuando cae la noche.', action: 'Encender', icon: Sparkles, accent: 'text-amber-300' },
  { id: 'canteen', name: 'Cantimplora del río', description: 'Recupera energía y permite cruzar corrientes profundas.', action: 'Beber agua', icon: CloudRain, accent: 'text-sky-300' },
]
...
function Home() {
  ...
  const handleUseObject = (object: typeof usableObjects[number]) => {
    setUsedObjects((current) => current.includes(object.id) ? current : [...current, object.id])
  ...
  return (
    <main className="min-h-dvh overflow-hidden bg-background text-foreground">
      ...
      <section id="objects" className="border-t border-border bg-card/30 px-5 py-24 lg:px-10 lg:py-32"><div className="mx-auto max-w-7xl"><div className="mb-12 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="mb-4 flex items-center gap-2 font-mono text-xs uppercase tracking-[0.24em] text-primary"><Backpack className="size-4" /> / Objetos utilizables</p><h2 className="font-serif text-4xl tracking-tight lg:text-6xl">Elige bien<br /><em className="text-muted-foreground">qué llevas contigo.</em></h2></div><p className="max-w-sm text-sm leading-relaxed text-muted-foreground">Cada objeto tiene un uso. Actívalos para cambiar tu expedición y revelar nuevas señales del mundo.</p></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{usableObjects.map((object) => { const Icon = object.icon; const used = usedObjects.includes(object.id); return <article key={object.id} className={`group rounded-2xl border p-5 transition-all duration-300 ${used ? 'border-primary/70 bg-primary/10 shadow-lg shadow-primary/10' : 'border-border bg-card hover:-translate-y-1 hover:border-primary/50'}`}><div className="flex items-start justify-between"><span className={`grid size-12 place-items-center rounded-xl border border-current/20 bg-background/40 ${object.accent}`}><Icon className="size-6" /></span><span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{used ? 'Activo' : 'Listo'}</span></div><h3 className="mt-7 font-serif text-xl">{object.name}</h3><p className="mt-2 min-h-12 text-xs leading-relaxed text-muted-foreground">{object.description}</p><button onClick={() => handleUseObject(object)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-full border border-primary/50 px-3 py-2 text-xs font-bold text-primary transition-all hover:bg-primary hover:text-primary-foreground active:scale-95">{object.action} <ArrowRight className="size-3" /></button></article> })}</div><p className="mt-5 text-center font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{usedObjects.length}/{usableObjects.length} objetos utilizados en esta expedición</p></div></section>
